...
if __name__ == '__main__':
    ...
    train_studies, train_annotation, train_counter = construct_studies(
        'data/data/lumbar_train150', 'data/data/lumbar_train150_annotation.json', multiprocessing=True)
    valid_studies, valid_annotation, valid_counter = construct_studies(
        'data/data/lumbar_train51/train', 'data/data/lumbar_train51_annotation.json', multiprocessing=True)
    ...
    train_dataloader = DisDataLoader(
        train_studies, train_annotation, batch_size=8, num_workers=8, num_rep=10, prob_rotate=1, max_angel=180,
        sagittal_size=dis_model.sagittal_size, transverse_size=dis_model.sagittal_size, k_nearest=0
    )

    valid_evaluator = Evaluator(
        dis_model, valid_studies, 'data/data/lumbar_train51_annotation.json', num_rep=30, max_dist=6,
    )
    ...
    fit_result = torch_utils.fit(
        ...
        resume='models/baseline_072401.dis_model'
    )

    torch.save(dis_model.cpu().state_dict(), 'models/baseline_072402.dis_model')
    # 预测
    testA_studies = construct_studies('data/data/lumbar_testA50/', multiprocessing=True)

    result = []
    for study in testA_studies.values():
        result.append(dis_model.eval()(study, True))

    with open('predictions/baseline_072402.json', 'w') as file:
